...
parse_transform(Forms, Options) ->
  case config(Forms) of
    ...
    Config ->
      ...
      CFileName = filename:join(OutDir, NIFId ++ ".c"),
      CCode = build_c_code(Module, Includes, CFunctions),
      ok = file:write_file(CFileName, CCode),
      io:format("cwd: ~p~nOptions: ~p~n~n", [file:get_cwd(), Options]),
      % Compile the C file
      SOName = filename:join(OutDir, NIFId ++ ".so"),
      Command = ["gcc -o ", SOName, " ", flags(Flags), " ", CFileName],
      _ = case os:cmd(Command) of
            [] -> ignore;
            Error -> io:format("Compiler Error:~n$~s~n> ~s", [Command, Error])
          end,
      % Make sure we have an init function and add the nif loading code
      {InitFunc, Forms3} = ensure_init_function(Module, Forms2),
      Forms4 = add_nif_loader(Forms3, InitFunc, filename:join(OutDir, NIFId)),
...
get_nif_loader(Line, NIF) ->
  M = {atom, Line, erlang},
  F = {atom, Line, load_nif},
  A = [{string, Line, NIF}, {integer, Line, 0}],
